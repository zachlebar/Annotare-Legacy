...
{"click .save":"save","click .discard":"discard","click .delete":"delete_note","keyup #edit-editor":"autosave"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.autosave=function(event){event.preventDefault();return localStorage[this.autosave_key()]=$("#edit-editor").val()};Edit.prototype.autosave_key=function(){return"autosave-draft-"+this.doc.id};Edit.prototype.render=function(){var context,_this=this;if(!this.query_params.id)return;
this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){console.log("Inside anonymous function...");if(ok){console.log("Restoring ...");return $("#edit-editor").val(localStorage[_this.autosave_key()])}else return delete localStorage[_this.autosave_key()]});
this.unbind_actions();this.bind_actions();return $("#edit-editor").autoResize({extraSpace:100,maxHeight:9E3})};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=$("#edit-editor").val();this.doc.save();ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();
return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok)return window.location.hash="#/list"})};Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};
Edit.prototype.loggin=function(event){event.preventDefault();return console.log(this.autosave_key())};return Edit}(Flakey.controllers.Controller);module.exports=Edit}).call(this)});
