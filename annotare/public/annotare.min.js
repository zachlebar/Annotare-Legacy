...
this.annotations.constructor!==Array)this.annotations=[];notes=[];_ref=this.annotations;for(_i=0,_len=_ref.length;_i<_len;_i++){id=_ref[_i];note=Annotation.get(id);if(note!=null)notes.push(note)}return notes};Document.prototype.render=function(){var classy_converter,converter,html;converter=new Showdown.converter;html=converter.makeHtml(this.base_text);classy_converter=new Classify.converter;html=classy_converter.addClasses(html);console.log(classy_converter.extractClass(html,"title"));return html};
return Document}(Flakey.models.Model);module.exports=Document}).call(this)});
