...
require.define("/controllers/annotare.js",function(require,module,exports,__dirname,__filename){(function(){var $,Detail,Edit,Flakey,List,Main,NewDocument,__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};Flakey=require("flakey");$=Flakey.$;NewDocument=require("./new_document");
List=require("./list");Detail=require("./detail");Edit=require("./edit");Main=function(_super){__extends(Main,_super);function Main(config){this.id="main-stack";this.class_name="stack";this.controllers={new_document:NewDocument,list:List,detail:Detail,edit:Edit};this.routes={"^/new$":"new_document","^/list$":"list","^/detail$":"detail","^/edit":"edit"};this["default"]="/list";Main.__super__.constructor.call(this,config)}return Main}(Flakey.controllers.Stack);module.exports=Main}).call(this)});
require.define("/controllers/new_document.js",function(require,module,exports,__dirname,__filename){(function(){var $,Document,Flakey,NewDocument,autoresize,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;
return child};Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");Document=require("../models/Document");NewDocument=function(_super){__extends(NewDocument,_super);function NewDocument(config){this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.id="new-document-view";this.class_name="new_document view";this.actions={"click .save":"save","click .discard":"discard"};NewDocument.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("new_document",
require("../views/new_document"))}NewDocument.prototype.render=function(){this.html(this.tmpl.render({}));this.unbind_actions();this.bind_actions();$("#editor").autoResize({extraSpace:100,maxHeight:2E3});return $("#name, #editor").blur()};NewDocument.prototype.save=function(params){var doc,name,text;name=$("#name").val();text=$("#editor").val();if(name.length>0&&text.length>0){doc=new Document({name:name,base_text:text});doc.generate_slug();doc.save();return window.location.hash="#/detail?"+Flakey.util.querystring.build({id:doc.id})}};
NewDocument.prototype.discard=function(params){if(confirm("Are you sure you want to discard all unsaved changes?"))return window.location.hash="#/list"};return NewDocument}(Flakey.controllers.Controller);module.exports=NewDocument}).call(this)});
...
"slug","base_text","annotations"];Document.prototype.annotate=function(selection,html,attachment){var note,type;if(attachment===void 0)type="highlight";else type="note";note=new Annotation({text:selection,type:type,attachment:attachment,document:this.id});note.save();if(!this.annotations||this.annotations.constructor!==Array)this.annotations=[];this.annotations.push(note.id);this.save();return note.apply(html)};Document.prototype.draw_annotations=function(html){var note,_i,_len,_ref;if(!this.annotations||
this.annotations.constructor!==Array)this.annotations=[];_ref=this.annotations;for(_i=0,_len=_ref.length;_i<_len;_i++){note=_ref[_i];html=note.apply(html)}return html};Document.prototype.generate_slug=function(){var slug;slug=this.name;slug=slug.toLowerCase().replace(/[^\_\ 0-9a-z-]/g,"").replace(/[ ]/g,"_");this.slug=slug;return slug};Document.prototype.get_notes=function(){var note,notes,_i,_len,_ref;if(!this.annotations||this.annotations.constructor!==Array)this.annotations=[];notes=[];_ref=this.annotations;
for(_i=0,_len=_ref.length;_i<_len;_i++){note=_ref[_i];if(note.type==="note")notes.push(note)}return notes};Document.prototype.render=function(){var converter,html;converter=new Showdown.converter;html=converter.makeHtml(this.base_text);return html};return Document}(Flakey.models.Model);module.exports=Document}).call(this)});
...
__obj.safe=__objSafe,__obj.escape=__escape;return __out.join("")}}).call(this)});
require.define("/controllers/detail.js",function(require,module,exports,__dirname,__filename){(function(){var $,Detail,Document,Flakey,apprise,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};
Flakey=require("flakey");$=Flakey.$;apprise=require("../lib/apprise-1.5.full");Document=require("../models/Document");Detail=function(_super){__extends(Detail,_super);function Detail(config){this.annotate=__bind(this.annotate,this);this.highlight=__bind(this.highlight,this);this.edit=__bind(this.edit,this);this.render=__bind(this.render,this);this.id="detail-view";this.class_name="detail view";this.actions={"click .edit":"edit","click .highlighter":"highlight","click .annotate":"annotate"};Detail.__super__.constructor.call(this,
config);this.tmpl=Flakey.templates.get_template("detail",require("../views/detail"))}Detail.prototype.render=function(){var context;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));this.unbind_actions();return this.bind_actions()};Detail.prototype.edit=function(event){return window.location.hash="#/edit?"+Flakey.util.querystring.build(this.query_params)};Detail.prototype.highlight=function(event){var html,selection;
selection=void 0;if(window.getSelection)selection=window.getSelection();else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return apprise("Please select some text first");else{html=$("#"+this.doc.slug).html();html=this.doc.annotate(selection,html);return $("#"+this.doc.slug).html(html)}};Detail.prototype.annotate=function(event){var html,options,selection,_this=this;selection=void 0;if(window.getSelection)selection=window.getSelection();
else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return apprise("Please select some text first");else{html=$("#"+this.doc.slug).html();options={input:true,animate:true};return apprise("Please enter a note",options,function(note){if(note){html=_this.doc.annotate(selection,html,note);return $("#"+_this.doc.slug).html(html)}})}};return Detail}(Flakey.controllers.Controller);module.exports=Detail}).call(this)});
...
require.define("/controllers/edit.js",function(require,module,exports,__dirname,__filename){(function(){var $,Document,Edit,Flakey,autoresize,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};
Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");Document=require("../models/Document");Edit=function(_super){__extends(Edit,_super);function Edit(config){this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.id="edit-view";this.class_name="edit_document view";this.actions={"click .save":"save","click .discard":"discard"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.render=
function(){var context;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));this.unbind_actions();this.bind_actions();return $("#editor").autoResize({extraSpace:100,maxHeight:1E4}).blur()};Edit.prototype.save=function(){this.doc.base_text=$("#editor").val();this.doc.save();return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(params){if(confirm("Are you sure you want to discard all unsaved changes?"))return window.location.hash=
"#/list"};return Edit}(Flakey.controllers.Controller);module.exports=Edit}).call(this)});
...
require.define("/index.js",function(require,module,exports,__dirname,__filename){(function(){var $,Annotare,App,Flakey,__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};Flakey=require("flakey");$=Flakey.$;Annotare=require("./controllers/annotare");App=function(_super){__extends(App,
_super);function App(){App.__super__.constructor.apply(this,arguments);this.annotare=new Annotare;this.append(this.annotare)}return App}(Flakey.controllers.Controller);$(document).ready(function(){var annotare,settings;settings={container:$("#application")};Flakey.init(settings);Flakey.models.backend_controller.sync("Document");Flakey.models.backend_controller.sync("Annotation");annotare=window.Annotare=new Annotare;return annotare.make_active()});module.exports=App}).call(this)});require("/index.js");
