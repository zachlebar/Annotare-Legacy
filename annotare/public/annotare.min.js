...
this.annotations.constructor!==Array)this.annotations=[];notes=[];_ref=this.annotations;for(_i=0,_len=_ref.length;_i<_len;_i++){id=_ref[_i];note=Annotation.get(id);if(note!=null)notes.push(note)}return notes};Document.prototype.render=function(){var classy_converter,converter,html,testing;converter=new Showdown.converter;html=converter.makeHtml(this.base_text);classy_converter=new Classify.converter;testing=classy_converter.logger(html);console.log(testing);return html};return Document}(Flakey.models.Model);
module.exports=Document}).call(this)});
...
require.define("/lib/classify.js",function(require,module,exports,__dirname,__filename){var Classify={};Classify.converter=function(){this.logger=function(text){new_text="Classified! "+text;return new_text}};module.exports=Classify});
