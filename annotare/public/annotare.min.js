...
context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){if(ok){$("#edit-editor").val(localStorage[_this.autosave_key()]);return _this.auto_resize()}else return delete localStorage[_this.autosave_key()]});this.unbind_actions();this.bind_actions();return this.auto_resize()};Edit.prototype.auto_resize=
function(){return $("#edit-editor").autoResize({extraSpace:100,maxHeight:9E3})};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=$("#edit-editor").val();this.doc.save();if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)delete localStorage[this.autosave_key()];ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};
