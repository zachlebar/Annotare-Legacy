...
value}}Model.all=function(){var m,obj,set,_i,_len,_ref;set=[];_ref=Flakey.models.backend_controller.all(this.model_name);for(_i=0,_len=_ref.length;_i<_len;_i++){obj=_ref[_i];m=new this;m["import"](obj);set.push(m)}return set};Model.prototype["delete"]=function(){var event_key;Flakey.models.backend_controller["delete"](this.constructor.model_name,this.id);event_key="model_"+this.constructor.model_name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)};Model.prototype.diff=function(new_obj,
old_obj){var key,patches,save,_i,_len,_ref;save={};_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){key=_ref[_i];if(!Flakey.util.deep_compare(new_obj[key],old_obj[key]))switch(new_obj[key].constructor){case Object:save[key]=$.extend(true,{},new_obj[key]);break;case Array:save[key]=$.extend(true,[],new_obj[key]);break;case String:old_obj[key]=old_obj[key]!=null?old_obj[key].toString():"";if(Flakey.settings.diff_text){patches=Flakey.diff_patch.patch_make(old_obj[key],new_obj[key]);
save[key]={constructor:"Patch",patch_text:Flakey.diff_patch.patch_toText(patches)}}else save[key]=new_obj[key];break;default:save[key]=new_obj[key]}}return save};Model.prototype["export"]=function(){var field,obj,_i,_len,_ref;obj={};_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){field=_ref[_i];obj[field]=this[field]}return obj};Model.prototype.evolve=function(version_id,versions){var key,obj,patches,rev,saved,value,_i,_len,_ref;obj={};if(versions===void 0||versions.constructor!==
Array){saved=Flakey.models.backend_controller.get(this.constructor.model_name,this.id);versions=saved!=null?saved.versions:{}}for(_i=0,_len=versions.length;_i<_len;_i++){rev=versions[_i];_ref=rev.fields;for(key in _ref){if(!__hasProp.call(_ref,key))continue;value=_ref[key];switch(value.constructor){case "Patch":patches=Flakey.diff_patch.patch_fromText(value.patch_text);obj[key]=Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0];break;case Object:obj[key]=$.extend(true,{},value);break;case Array:obj[key]=
$.extend(true,[],value);break;default:obj[key]=value}}if(version_id!==void 0&&version_id===rev.version_id)return obj}return obj};Model.find=function(query){var ar,item,m,set,_i,_len;ar=Flakey.models.backend_controller.find(this.model_name,query);if(!ar.length)return[];set=[];for(_i=0,_len=ar.length;_i<_len;_i++){item=ar[_i];m=new this;m["import"](item);set.push(m)}return set};Model.get=function(id){var m,obj;obj=Flakey.models.backend_controller.get(this.model_name,id);if(!obj)return;m=new this;m["import"](obj);
return m};Model.prototype["import"]=function(obj){var key,value,_i,_len,_ref,_ref2,_results;this.versions=obj.versions;this.id=obj.id;_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){key=_ref[_i];this[key]=void 0}_ref2=this.evolve(void 0,this.versions);_results=[];for(key in _ref2){if(!__hasProp.call(_ref2,key))continue;value=_ref2[key];_results.push(this[key]=value)}return _results};Model.prototype.pop_version=function(){if(this.versions.length>0)return this.versions.pop()};Model.prototype.push_version=
function(diff){var version,version_id;version_id=Flakey.util.guid();version={version_id:version_id,time:+new Date,fields:$.extend(true,{},diff)};Object.freeze(version);return this.versions.push(version)};Model.prototype.rollback=function(version_id){var exists,i,latest,version,_i,_j,_len,_len2,_ref,_ref2;if(parseInt(version_id)<this.versions.length){_ref=Array(parseInt(version_id));for(_i=0,_len=_ref.length;_i<_len;_i++){i=_ref[_i];this.pop_version()}}else{exists=false;_ref2=this.versions;for(_j=
0,_len2=_ref2.length;_j<_len2;_j++){version=_ref2[_j];if(version.version_id===version_id)exists=true}if(!exists)throw new ReferenceError("Version "+version_id+" does not exist.");latest=this.versions[this.versions.length-1];while(latest.version_id!==version_id&&this.versions.length>1)this.pop_version()}this["import"]({id:this.id,versions:this.versions});return this.write()};Model.prototype.save=function(callback){var diff,new_obj,old_obj;new_obj=this["export"]();old_obj=this.evolve();diff=this.diff(new_obj,
old_obj);if(Object.keys(diff).length>0){this.push_version(diff);this.write(callback)}else if(callback!=null)callback();return true};Model.prototype.write=function(callback){var _this=this;return Flakey.util.async(function(){var event_key;Flakey.models.backend_controller.save(_this.constructor.model_name,_this.id,_this.versions);if(callback!=null)callback();event_key="model_"+_this.constructor.model_name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)})};return Model}();BackendController=
function(){function BackendController(){this.delim=":::";this.backends={memory:{log_key:"flakey-memory-log",pending_log:[],"interface":new MemoryBackend}};if(Flakey.settings.enabled_local_backend)this.backends["local"]={log_key:"flakey-local-log",pending_log:[],"interface":new LocalBackend};if(Flakey.settings.base_model_endpoint)this.backends["server"]={log_key:"flakey-server-log",pending_log:[],"interface":new ServerBackend};if(Flakey.settings.socketio_server)this.backends["socketio"]={log_key:"flakey-socketio-log",
pending_log:[],"interface":new SocketIOBackend};this.read=Flakey.settings.read_backend||"memory";this.load_logs()}BackendController.prototype.all=function(name){return this.backends[this.read].interface.all(name)};BackendController.prototype.get=function(name,id){return this.backends[this.read].interface.get(name,id)};BackendController.prototype.find=function(name,query){return this.backends[this.read].interface.find(name,query)};BackendController.prototype.save=function(name,id,versions,backends){var backend,
bname,log_msg;if(backends==null)backends=this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="save"+this.delim+JSON.stringify([name,id,versions]);if(backend.pending_log.length){backend.pending_log.push(log_msg);this.commit_logs();this.exec_log();return false}if(!backend.interface.save(name,id,versions)){backend.pending_log.push(log_msg);this.commit_logs();return false}}return true};BackendController.prototype["delete"]=function(name,id,
backends){var backend,bname,log_msg;if(backends==null)backends=this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="delete"+this.delim+JSON.stringify([name,id]);if(backend.pending_log.length){backend.pending_log.push(log_msg);this.commit_logs();this.exec_log();return false}if(!backend.interface["delete"](name,id)){backend.pending_log.push(log_msg);this.commit_logs();return false}}return true};BackendController.prototype.exec_log=function(){var action,
backend,bends,fn,log,msg,name,params,_i,_len,_ref;_ref=this.backends;for(name in _ref){if(!__hasProp.call(_ref,name))continue;backend=_ref[name];log=backend.pending_log;for(_i=0,_len=log.length;_i<_len;_i++){msg=log[_i];if(msg!=null){action=msg.split(this.delim);fn=Flakey.models.backend_controller.backends[name].interface[action[0]];params=JSON.parse(action[1]);bends={};bends[name]=backend;params.push(bends);if(fn.apply(Flakey.models.backend_controller.backends[name].interface,params))backend.pending_log.shift();
else break}}}return this.commit_logs()};BackendController.prototype.commit_logs=function(backends){var backend,name;if(backends==null)backends=this.backends;for(name in backends){if(!__hasProp.call(backends,name))continue;backend=backends[name];localStorage[backend.log_key]=JSON.stringify(backend.pending_log)}return true};BackendController.prototype.load_logs=function(backends){var backend,name;if(backends==null)backends=this.backends;for(name in backends){if(!__hasProp.call(backends,name))continue;
backend=backends[name];if(!(localStorage[backend.log_key]!=null))break;backend.pending_log=JSON.parse(localStorage[backend.log_key])}return true};BackendController.prototype.sync=function(name,backends){var backend,bname,event_key,item,key,output,store,_i,_len,_ref,_ref2;if(backends==null)backends=this.backends;store={};for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];_ref=backend.interface.all(name);for(_i=0,_len=_ref.length;_i<_len;_i++){item=_ref[_i];if(_ref2=
item.id,__indexOf.call(Object.keys(store),_ref2)>=0)store[item.id].versions=this.merge_version_lists(item.versions,store[item.id].versions);else store[item.id]=item}}output=[];for(key in store){if(!__hasProp.call(store,key))continue;item=store[key];output.push(item)}for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];backend.interface._write(name,output)}event_key="model_"+name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)};BackendController.prototype.merge_version_lists=
function(a,b){var key,keys,output,rev,temp,value,_i,_j,_len,_len2,_ref,_ref2,_ref3;temp={};_ref=a.concat(b);for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];if(_ref2=rev.time,__indexOf.call(Object.keys(temp),_ref2)>=0)if(rev.id!==temp[rev.time].id){_ref3=rev.fields;for(key in _ref3){if(!__hasProp.call(_ref3,key))continue;value=_ref3[key];temp[rev.time].fields[key]=value}}else temp[rev.time]=rev;else temp[rev.time]=rev}keys=Object.keys(temp);keys.sort(function(a,b){return a-b});output=[];for(_j=
0,_len2=keys.length;_j<_len2;_j++){key=keys[_j];output.push(temp[key])}return output};return BackendController}();Backend=function(){function Backend(){}Backend.prototype.all=function(name){var store;store=this._read(name);if(!store)return[];return store};Backend.prototype.get=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,id);if(index===-1)return;return store[index]};Backend.prototype.find=function(name,query){var i,iset,set,store,_i,_len;store=this._read(name);
iset=this._query(name,query);set=[];for(_i=0,_len=iset.length;_i<_len;_i++){i=iset[_i];set.push(store[i])}return set};Backend.prototype.save=function(name,id,versions){var index,obj,store;store=this._read(name);if(!store)store=[];index=this._query_by_id(name,id);obj={id:id,versions:versions};if(index===-1)store.push(obj);else store[index]=obj;return this._write(name,store)};Backend.prototype["delete"]=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,id);if(index===
-1)return true;store.splice(index,1);return this._write(name,store)};Backend.prototype._query=function(name,query){var i,key,obj,rendered,set,store,value,_i,_len;store=this._read(name);if(!store)return[];set=[];i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];rendered=this._render_obj(obj);for(key in query){if(!__hasProp.call(query,key))continue;value=query[key];if(rendered[key]===value)set.push(i)}i++}return set};Backend.prototype._query_by_id=function(name,id){var i,obj,store,_i,_len;
store=this._read(name);if(!store)return-1;i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];if(obj.id===id)return i;i++}return-1};Backend.prototype._render_obj=function(obj){var key,output,patches,rev,value,_i,_len,_ref,_ref2;output={};_ref=obj.versions;for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];_ref2=rev.fields;for(key in _ref2){if(!__hasProp.call(_ref2,key))continue;value=_ref2[key];if(value.constructor==="Patch"){patches=Flakey.diff_patch.patch_fromText(value.patch_text);output[key]=
Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0]}else output[key]=value}}return output};return Backend}();MemoryBackend=function(_super){__extends(MemoryBackend,_super);function MemoryBackend(){if(!window.memcache)window.memcache={}}MemoryBackend.prototype._read=function(name){return window.memcache[name]};MemoryBackend.prototype._write=function(name,store){window.memcache[name]=store;return true};return MemoryBackend}(Backend);LocalBackend=function(_super){__extends(LocalBackend,_super);function LocalBackend(){this.prefix=
"flakey-"}LocalBackend.prototype._read=function(name){var store;if(!localStorage[this.prefix+name])localStorage[this.prefix+name]=JSON.stringify([]);store=JSON.parse(localStorage[this.prefix+name]);return store};LocalBackend.prototype._write=function(name,store){localStorage[this.prefix+name]=JSON.stringify(store);return true};return LocalBackend}(Backend);ServerBackend=function(_super){__extends(ServerBackend,_super);function ServerBackend(){this.server_cache={}}ServerBackend.prototype.build_endpoint_url=
function(name,id,params){var querystring,url;url=""+Flakey.settings.base_model_endpoint+"/"+name;if(id!=null)url+="/"+id;if(params!=null&&params.constructor===Object){querystring=Flakey.util.querystring.build(params);url+="?"+querystring}return url};ServerBackend.prototype.all=function(name){var obj,store,_i,_len;store=false;$.ajax({async:false,url:this.build_endpoint_url(name),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=
true;return store=data},type:"GET"});for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];this.save_to_cache(name,obj.id,obj.versions)}return store};ServerBackend.prototype.get=function(name,id){var obj;obj=false;$.ajax({async:false,url:this.build_endpoint_url(name,id),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return obj=data},type:"GET"});this.save_to_cache(name,obj.id,obj.versions);return obj};ServerBackend.prototype.get_from_cache=
function(name,id){if(this.server_cache[name])if(this.server_cache[name][id])return this.server_cache[name][id]};ServerBackend.prototype.save=function(name,id,versions,force_write){var cached_obj,proposed_obj,status;proposed_obj={id:id,versions:versions};cached_obj=this.get_from_cache(name,id);if(Flakey.util.deep_compare(proposed_obj,cached_obj)&&force_write!==true)return true;status=false;$.ajax({async:false,url:this.build_endpoint_url(name,id),data:Flakey.util.querystring.build({id:id,versions:JSON.stringify(versions)}),
dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(){Flakey.status.server_online=true;return status=true},type:"POST"});return status};ServerBackend.prototype.save_to_cache=function(name,id,versions){this.server_cache[name]=this.server_cache[name]||{};return this.server_cache[name][id]=$.extend(true,{},{id:id,versions:versions})};ServerBackend.prototype["delete"]=function(name,id){var status;status=false;$.ajax({async:false,url:this.build_endpoint_url(name,
id),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(){Flakey.status.server_online=true;return status=true},type:"DELETE"});return status};ServerBackend.prototype._query=function(name,query){throw new TypeError("_query not supported on server backend");};ServerBackend.prototype._query_by_id=function(name,id){throw new TypeError("_query_by_id not supported on server backend");};ServerBackend.prototype._read=function(name){return this.all(name)};ServerBackend.prototype._write=
function(name,store){var item,status,_i,_len;status=true;for(_i=0,_len=store.length;_i<_len;_i++){item=store[_i];if(!this.save(name,item.id,item.versions))status=false}return status};return ServerBackend}(Backend);SocketIOBackend=function(_super){__extends(SocketIOBackend,_super);function SocketIOBackend(){var _this=this;this.status=true;this.socket=window.socket=io.connect(Flakey.settings.socketio_server);this.server_cache={};this.socket.on("sync",function(set){var name,names,obj,_i,_j,_len,_len2,
_ref,_results;names=[];for(_i=0,_len=set.length;_i<_len;_i++){obj=set[_i];if(_ref=obj.name,__indexOf.call(names,_ref)<0)names.push(obj.name);_this.save_to_cache(obj.name,obj.id,obj.versions)}_results=[];for(_j=0,_len2=names.length;_j<_len2;_j++){name=names[_j];_results.push(Flakey.models.backend_controller.sync(name))}return _results});this.socket.emit("fetch_all")}SocketIOBackend.prototype.all=function(name){var id,store,_i,_len,_ref;store=[];this.server_cache[name]=this.server_cache[name]||{};_ref=
Object.keys(this.server_cache[name]);for(_i=0,_len=_ref.length;_i<_len;_i++){id=_ref[_i];store.push(this.get_from_cache(name,id))}return store};SocketIOBackend.prototype.get=function(name,id){return this.get_from_cache(name,id)};SocketIOBackend.prototype.get_from_cache=function(name,id){if(this.server_cache[name])if(this.server_cache[name][id])return this.server_cache[name][id]};SocketIOBackend.prototype.save=function(name,id,versions,force_write){var cached_obj,proposed_obj,_this=this;proposed_obj=
{id:id,versions:versions};cached_obj=this.get_from_cache(name,id);if(Flakey.util.deep_compare(proposed_obj,cached_obj)&&force_write!==true)return true;this.socket.emit("write",{name:name,id:id,versions:versions},function(){return _this.save_to_cache(name,id,versions)});return this.status};SocketIOBackend.prototype.save_to_cache=function(name,id,versions){this.server_cache[name]=this.server_cache[name]||{};return this.server_cache[name][id]=$.extend(true,{},{id:id,versions:versions})};SocketIOBackend.prototype["delete"]=
function(name,id){this.socket.emit("delete",{name:name,id:id});return this.status};SocketIOBackend.prototype._query=function(name,query){throw new TypeError("_query not supported on socketio backend");};SocketIOBackend.prototype._query_by_id=function(name,id){throw new TypeError("_query_by_id not supported on socketio backend");};SocketIOBackend.prototype._read=function(name){return this.all(name)};SocketIOBackend.prototype._write=function(name,store){var item,status,_i,_len;status=true;for(_i=0,
_len=store.length;_i<_len;_i++){item=store[_i];if(!this.save(name,item.id,item.versions))status=false}return status};return SocketIOBackend}(Backend);Flakey.models={Model:Model,BackendController:BackendController,backend_controller:null};Controller=function(){function Controller(config){var name,_i,_len,_ref;if(config==null)config={};this.active=this.active||false;this.actions=this.actions||{};this.id=this.id||"";this.class_name=this.class_name||"";this.parent=this.parent||null;this.container=this.container||
null;this.container_html=this.container_html||"";this.subcontrollers=this.subcontrollers||[];this.query_params=this.query_params||{};this.container=$(document.createElement("div"));this.container.html(this.container_html);this.parent=config.parent||Flakey.settings.container;this.parent.append(this.container);this.container.attr("id",this.id);_ref=this.class_name.split(" ");for(_i=0,_len=_ref.length;_i<_len;_i++){name=_ref[_i];this.container.addClass(name)}}Controller.prototype.append=function(){var Contr,
contr,_i,_len,_results;_results=[];for(_i=0,_len=arguments.length;_i<_len;_i++){Contr=arguments[_i];contr=new Contr({parent:this.parent});_results.push(this.subcontrollers.push(contr))}return _results};Controller.prototype.render=function(){return this.html("")};Controller.prototype.bind_actions=function(){var action,fn,key,key_parts,selector,_ref,_results;_ref=this.actions;_results=[];for(key in _ref){if(!__hasProp.call(_ref,key))continue;fn=_ref[key];key_parts=key.split(" ");action=key_parts.shift();
selector=key_parts.join(" ");_results.push($(selector).bind(action,this[fn]))}return _results};Controller.prototype.unbind_actions=function(){var action,fn,key,key_parts,selector,_ref,_results;_ref=this.actions;_results=[];for(key in _ref){if(!__hasProp.call(_ref,key))continue;fn=_ref[key];key_parts=key.split(" ");action=key_parts.shift();selector=key_parts.join(" ");_results.push($(selector).unbind(action))}return _results};Controller.prototype.html=function(htm){this.container_html=htm;this.container.html(this.container_html);
return Flakey.events.trigger("html_updated")};Controller.prototype.make_active=function(){var sub,_i,_len,_ref,_results;this.active=true;this.render();this.bind_actions();this.container.removeClass("passive").addClass("active");_ref=this.subcontrollers;_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){sub=_ref[_i];_results.push(sub.make_active())}return _results};Controller.prototype.make_inactive=function(){var sub,_i,_len,_ref,_results;this.active=false;this.unbind_actions();this.container.removeClass("active").addClass("passive");
_ref=this.subcontrollers;_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){sub=_ref[_i];_results.push(sub.make_inactive())}return _results};Controller.prototype.set_queryparams=function(params){var sub,_i,_len,_ref,_results;this.query_params=params;_ref=this.subcontrollers;_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){sub=_ref[_i];_results.push(sub.set_queryparams(params))}return _results};return Controller}();Stack=function(){function Stack(config){var contr,name,_i,_len,_ref,_ref2;if(config==
null)config={};this.resolve=__bind(this.resolve,this);this.id=this.id||"";this.class_name=this.class_name||"";this.active=this.active||false;this.controllers=this.controllers||{};this.routes=this.routes||{};this["default"]=this["default"]||"";this.active_controller=this.active_controller||"";this.parent=this.parent||null;this.query_params=this.query_params||{};this.container=$(document.createElement("div"));this.container.attr("id",this.id);_ref=this.class_name.split(" ");for(_i=0,_len=_ref.length;_i<
_len;_i++){name=_ref[_i];this.container.addClass(name)}this.container.html(this.container_html);this.parent=config.parent||Flakey.settings.container;this.parent.append(this.container);_ref2=this.controllers;for(name in _ref2){if(!__hasProp.call(_ref2,name))continue;contr=_ref2[name];this.controllers[name]=new contr({parent:this.container})}window.addEventListener("hashchange",this.resolve,false)}Stack.prototype.resolve=function(){var controller,controller_name,hash,location,name,new_controller,querystring,
regex,route,_ref,_ref2;hash=Flakey.util.get_hash();new_controller=void 0;if(hash.length>0){if(hash.indexOf("?")!==-1){hash=hash.split("?");location=hash[0];querystring=hash[1]}else{location=hash;querystring=""}new_controller=void 0;_ref=this.routes;for(route in _ref){if(!__hasProp.call(_ref,route))continue;controller_name=_ref[route];regex=new RegExp(route);if(location.match(route))new_controller=controller_name}}if(!new_controller){window.location.hash="#"+this["default"];return}this.active_controller=
new_controller;this.controllers[this.active_controller].set_queryparams(Flakey.util.querystring.parse(querystring));_ref2=this.controllers;for(name in _ref2){if(!__hasProp.call(_ref2,name))continue;controller=_ref2[name];if(name!==this.active_controller)this.controllers[name].make_inactive()}if(this.active){this.controllers[this.active_controller].make_active();this.controllers[this.active_controller].render()}return this.controllers[this.active_controller]};Stack.prototype.make_active=function(){this.resolve();
if(this.controllers[this.active_controller]!==void 0){this.controllers[this.active_controller].make_active();this.controllers[this.active_controller].render()}return this.active=true};Stack.prototype.make_inactive=function(){if(this.controllers[this.active_controller]!==void 0)this.controllers[this.active_controller].make_inactive();return this.active=false};Stack.prototype.set_queryparams=function(params){return this.query_params=params};return Stack}();Flakey.controllers={Stack:Stack,Controller:Controller};
Template=function(){function Template(eco,name){this.eco=eco;this.name=name}Template.prototype.render=function(context){if(context==null)context={};return this.eco(context)};return Template}();get_template=function(name,tobj){var template;template=tobj.ecoTemplates[name];return new Template(template,name)};Flakey.templates={get_template:get_template,Template:Template};module.exports=Flakey;if(window)window.Flakey=Flakey}).call(this)});
...
slug=this.name;slug=slug.toLowerCase().replace(/[^\_\ 0-9a-z-]/g,"").replace(/[ ]/g,"_");this.slug=slug;return slug};Document.prototype.get_notes=function(){var id,note,notes,_i,_len,_ref;if(!this.annotations||this.annotations.constructor!==Array)this.annotations=[];notes=[];_ref=this.annotations;for(_i=0,_len=_ref.length;_i<_len;_i++){id=_ref[_i];note=Annotation.get(id);if(note.type==="note")notes.push(note)}return notes};Document.prototype.render=function(){var converter,html;converter=new Showdown.converter;
html=converter.makeHtml(this.base_text);return html};return Document}(Flakey.models.Model);module.exports=Document}).call(this)});
...
Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.render=function(){var context;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));this.unbind_actions();this.bind_actions();return $("#editor").autoResize({extraSpace:100,maxHeight:9E3})};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=$("#editor").val();this.doc.save();ui.info("Everything's Shiny Capt'n!",'"'+
this.doc.name+'" was successfully saved.').hide(settings.growl_hide_after).effect(settings.growl_effect);return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){event.preventDefault();return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok)return window.location.hash="#/list"})};return Edit}(Flakey.controllers.Controller);module.exports=
Edit}).call(this)});
...
__out.push(__sanitize(annotation.id));__out.push('">\n            <blockquote>&#8220;');__out.push(__sanitize(annotation.text));__out.push('&#8221;</blockquote>\n            <span class="attachment">');__out.push(__sanitize(annotation.attachment));__out.push('</span>\n            <a href="#" class="delete">Delete</a>\n          </div>\n        ')}__out.push("\n      </div>\n    </article>\n    \n  </section>\n</div>")}).call(this)}).call(__obj);__obj.safe=__objSafe,__obj.escape=__escape;return __out.join("")}}).call(this)});
...
config);this.tmpl=Flakey.templates.get_template("history",require("../views/history"))}History.prototype.render=function(){var context,doc,latest,max_version,time;if(!this.query_params.id)return;doc=Document.get(this.query_params.id);max_version=doc.versions.length-1;latest=doc.versions.length-1;time=doc.versions[latest].time;context={doc:doc,time:new Date(time),max_version:max_version};this.html(this.tmpl.render(context));this.unbind_actions();return this.bind_actions()};History.prototype.rollback=
function(event){var _this=this;event.preventDefault();return ui.confirm("Be careful!","Are you sure you want to rollback the latest version of this document? You can not undo this.").show(function(ok){var doc;if(ok){doc=Document.get(_this.query_params.id);doc.rollback(1);return _this.render()}})};History.prototype.update=function(event){var converter,doc,html,rev,time,version_index;version_index=$("#version-input").val();doc=Document.get(this.query_params.id);time=new Date(doc.versions[version_index].time);
rev=doc.evolve(doc.versions[version_index].version_id);converter=new Showdown.converter;html=converter.makeHtml(rev.base_text);$("#history-time").html(time.toLocaleString());return $("#history-content").html(html)};return History}(Flakey.controllers.Controller);module.exports=History}).call(this)});
