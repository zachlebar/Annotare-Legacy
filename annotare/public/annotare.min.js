...
config);this.tmpl=Flakey.templates.get_template("new_document",require("../views/new_document"))}NewDocument.prototype.render=function(){this.html(this.tmpl.render({title:this.query_params.title}));this.unbind_actions();this.bind_actions();$("#new-doc-editor").autoResize({extraSpace:100,maxHeight:2E3});return $("#new-doc-editor").blur()};NewDocument.prototype.save=function(params){var class_converter,doc,html,text;text=$("#new-doc-editor").val();if(text.length>0){doc=new Document({base_text:text});
html=doc.render();class_converter=new Classify.converter;doc.name=class_converter.extractClass(html,"title");doc.save();doc.generate_slug();ui.info("Everything's Shiny Capt'n!",'"'+doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build({id:doc.id})}};NewDocument.prototype.discard=function(params){return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard this document?").show(function(ok){if(ok)return window.location.hash=
"#/list"})};return NewDocument}(Flakey.controllers.Controller);module.exports=NewDocument}).call(this)});
...
if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){if(ok){$("#edit-editor").val(localStorage[_this.autosave_key()]);return _this.auto_resize()}else return delete localStorage[_this.autosave_key()]});this.unbind_actions();
this.bind_actions();return this.auto_resize()};Edit.prototype.auto_resize=function(){return $("#edit-editor").autoResize({extraSpace:100,maxHeight:9E3})};Edit.prototype.save=function(event){var class_converter,tmp_html;event.preventDefault();this.doc.base_text=$("#edit-editor").val();tmp_html=this.doc.render();class_converter=new Classify.converter;this.doc.name=class_converter.extractClass(tmp_html,"title");this.doc.save();if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>
0)delete localStorage[this.autosave_key()];ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){});if(ok)return window.location.hash="#/list"};
