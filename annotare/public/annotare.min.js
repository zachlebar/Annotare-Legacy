...
obj;obj=Flakey.models.backend_controller.get(this.model_name,id);if(!obj)return;m=new this;m["import"](obj);return m};Model.prototype["delete"]=function(){var event_key;Flakey.models.backend_controller["delete"](this.constructor.model_name,this.id);event_key="model_"+this.constructor.model_name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)};Model.prototype.rollback=function(version_id){var exists,i,latest,version,_i,_j,_len,_len2,_ref,_ref2;if(parseInt(version_id)<this.versions.length){_ref=
Array(parseInt(version_id));for(_i=0,_len=_ref.length;_i<_len;_i++){i=_ref[_i];this.pop_version()}}else{exists=false;_ref2=this.versions;for(_j=0,_len2=_ref2.length;_j<_len2;_j++){version=_ref2[_j];if(version.version_id===version_id)exists=true}if(!exists)throw new ReferenceError("Version "+version_id+" does not exist.");latest=this.versions[this.versions.length-1];while(latest.version_id!==version_id&&this.versions.length>1){this.pop_version();latest=this.versions[this.versions.length-1]}}this["import"]({id:this.id,
versions:this.versions});return this.write()};Model.prototype.save=function(callback){var diff,new_obj,old_obj;new_obj=this["export"]();old_obj=this.evolve();diff=this.diff(new_obj,old_obj);if(Object.keys(diff).length>0){this.push_version(diff);this.write(callback)}else if(callback!=null)callback();return true};Model.prototype.diff=function(new_obj,old_obj){var key,patches,save,_i,_len,_ref;save={};_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){key=_ref[_i];if(!Flakey.util.deep_compare(new_obj[key],
old_obj[key]))switch(new_obj[key].constructor){case Object:save[key]=$.extend(true,{},new_obj[key]);break;case Array:save[key]=$.extend(true,[],new_obj[key]);break;case String:old_obj[key]=old_obj[key]!=null?old_obj[key].toString():"";if(Flakey.settings.diff_text){patches=Flakey.diff_patch.patch_make(old_obj[key],new_obj[key]);save[key]={constructor:"Patch",patch_text:Flakey.diff_patch.patch_toText(patches)}}else save[key]=new_obj[key];break;default:save[key]=new_obj[key]}}return save};Model.prototype["export"]=
function(){var field,obj,_i,_len,_ref;obj={};_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){field=_ref[_i];obj[field]=this[field]}return obj};Model.prototype.evolve=function(version_id,versions){var key,obj,patches,rev,saved,value,_i,_len,_ref;obj={};if(versions===void 0||versions.constructor!==Array){saved=Flakey.models.backend_controller.get(this.constructor.model_name,this.id);versions=saved!=null?saved.versions:{}}for(_i=0,_len=versions.length;_i<_len;_i++){rev=versions[_i];
_ref=rev.fields;for(key in _ref){if(!__hasProp.call(_ref,key))continue;value=_ref[key];switch(value.constructor){case "Patch":patches=Flakey.diff_patch.patch_fromText(value.patch_text);obj[key]=Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0];break;case Object:obj[key]=$.extend(true,{},value);break;case Array:obj[key]=$.extend(true,[],value);break;default:obj[key]=value}}if(version_id!==void 0&&version_id===rev.version_id)return obj}return obj};Model.prototype["import"]=function(obj){var key,
value,_i,_len,_ref,_ref2,_results;this.versions=obj.versions;this.id=obj.id;_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){key=_ref[_i];this[key]=void 0}_ref2=this.evolve(void 0,this.versions);_results=[];for(key in _ref2){if(!__hasProp.call(_ref2,key))continue;value=_ref2[key];_results.push(this[key]=value)}return _results};Model.prototype.pop_version=function(){if(this.versions.length>0)return this.versions.pop()};Model.prototype.push_version=function(diff){var version,version_id;
version_id=Flakey.util.guid();version={version_id:version_id,time:+new Date,fields:$.extend(true,{},diff)};Object.freeze(version);return this.versions.push(version)};Model.prototype.write=function(callback){var _this=this;return Flakey.util.async(function(){var event_key;Flakey.models.backend_controller.save(_this.constructor.model_name,_this.id,_this.versions);if(callback!=null)callback();event_key="model_"+_this.constructor.model_name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,
void 0)})};return Model}();BackendController=function(){function BackendController(){this.delim=":::";this.backends={memory:{log_key:"flakey-memory-log",pending_log:[],"interface":new MemoryBackend}};if(Flakey.settings.enabled_local_backend)this.backends["local"]={log_key:"flakey-local-log",pending_log:[],"interface":new LocalBackend};if(Flakey.settings.base_model_endpoint)this.backends["server"]={log_key:"flakey-server-log",pending_log:[],"interface":new ServerBackend};if(Flakey.settings.socketio_server)this.backends["socketio"]=
{log_key:"flakey-socketio-log",pending_log:[],"interface":new SocketIOBackend};this.read=Flakey.settings.read_backend||"memory";this.load_logs()}BackendController.prototype.all=function(name){return this.backends[this.read].interface.all(name)};BackendController.prototype.get=function(name,id){return this.backends[this.read].interface.get(name,id)};BackendController.prototype.find=function(name,query){return this.backends[this.read].interface.find(name,query)};BackendController.prototype.save=function(name,
id,versions,backends){var backend,bname,log_msg;if(backends==null)backends=this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="save"+this.delim+JSON.stringify([name,id,versions]);if(backend.pending_log.length){backend.pending_log.push(log_msg);this.commit_logs();this.exec_log();return false}if(!backend.interface.save(name,id,versions)){backend.pending_log.push(log_msg);this.commit_logs();return false}}return true};BackendController.prototype["delete"]=
function(name,id,backends){var backend,bname,log_msg;if(backends==null)backends=this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="delete"+this.delim+JSON.stringify([name,id]);if(backend.pending_log.length){backend.pending_log.push(log_msg);this.commit_logs();this.exec_log();return false}if(!backend.interface["delete"](name,id)){backend.pending_log.push(log_msg);this.commit_logs();return false}}return true};BackendController.prototype.exec_log=
function(){var action,backend,bends,fn,log,msg,name,params,_i,_len,_ref;_ref=this.backends;for(name in _ref){if(!__hasProp.call(_ref,name))continue;backend=_ref[name];log=backend.pending_log;for(_i=0,_len=log.length;_i<_len;_i++){msg=log[_i];if(msg!=null){action=msg.split(this.delim);fn=Flakey.models.backend_controller.backends[name].interface[action[0]];params=JSON.parse(action[1]);bends={};bends[name]=backend;params.push(bends);if(fn.apply(Flakey.models.backend_controller.backends[name].interface,
...
query){var i,iset,set,store,_i,_len;store=this._read(name);iset=this._query(name,query);set=[];for(_i=0,_len=iset.length;_i<_len;_i++){i=iset[_i];set.push(store[i])}return set};Backend.prototype.save=function(name,id,versions){var index,obj,store;store=this._read(name);if(!store)store=[];index=this._query_by_id(name,id);obj={id:id,versions:versions};if(index===-1)store.push(obj);else store[index]=obj;return this._write(name,store)};Backend.prototype["delete"]=function(name,id){var index,store;store=
this._read(name);index=this._query_by_id(name,id);if(index===-1)return true;store.splice(index,1);return this._write(name,store)};Backend.prototype._query=function(name,query){var i,key,obj,rendered,set,store,value,_i,_len;store=this._read(name);if(!store)return[];set=[];i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];rendered=this._render_obj(obj);for(key in query){if(!__hasProp.call(query,key))continue;value=query[key];if(rendered[key]===value)set.push(i)}i++}return set};Backend.prototype._query_by_id=
function(name,id){var i,obj,store,_i,_len;store=this._read(name);if(!store)return-1;i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];if(obj.id===id)return i;i++}return-1};Backend.prototype._render_obj=function(obj){var key,output,patches,rev,value,_i,_len,_ref,_ref2;output={};_ref=obj.versions;for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];_ref2=rev.fields;for(key in _ref2){if(!__hasProp.call(_ref2,key))continue;value=_ref2[key];if(value.constructor==="Patch"){patches=Flakey.diff_patch.patch_fromText(value.patch_text);
output[key]=Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0]}else output[key]=value}}return output};return Backend}();MemoryBackend=function(_super){__extends(MemoryBackend,_super);function MemoryBackend(){if(!window.memcache)window.memcache={}}MemoryBackend.prototype._read=function(name){return window.memcache[name]};MemoryBackend.prototype._write=function(name,store){window.memcache[name]=store;return true};return MemoryBackend}(Backend);LocalBackend=function(_super){__extends(LocalBackend,
_super);function LocalBackend(){this.prefix="flakey-"}LocalBackend.prototype._read=function(name){var store;if(!localStorage[this.prefix+name])localStorage[this.prefix+name]=JSON.stringify([]);store=JSON.parse(localStorage[this.prefix+name]);return store};LocalBackend.prototype._write=function(name,store){localStorage[this.prefix+name]=JSON.stringify(store);return true};return LocalBackend}(Backend);ServerBackend=function(_super){__extends(ServerBackend,_super);function ServerBackend(){this.server_cache=
{}}ServerBackend.prototype.build_endpoint_url=function(name,id,params){var querystring,url;url=""+Flakey.settings.base_model_endpoint+"/"+name;if(id!=null)url+="/"+id;if(params!=null&&params.constructor===Object){querystring=Flakey.util.querystring.build(params);url+="?"+querystring}return url};ServerBackend.prototype.all=function(name){var obj,store,_i,_len;store=false;$.ajax({async:false,url:this.build_endpoint_url(name),dataType:"json",error:function(){return Flakey.status.server_online=false},
success:function(data){Flakey.status.server_online=true;return store=data},type:"GET"});for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];this.save_to_cache(name,obj.id,obj.versions)}return store};ServerBackend.prototype.get=function(name,id){var obj;obj=false;$.ajax({async:false,url:this.build_endpoint_url(name,id),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return obj=data},type:"GET"});this.save_to_cache(name,
obj.id,obj.versions);return obj};ServerBackend.prototype.get_from_cache=function(name,id){if(this.server_cache[name])if(this.server_cache[name][id])return this.server_cache[name][id]};ServerBackend.prototype.save=function(name,id,versions,force_write){var cached_obj,proposed_obj,status;proposed_obj={id:id,versions:versions};cached_obj=this.get_from_cache(name,id);if(Flakey.util.deep_compare(proposed_obj,cached_obj)&&force_write!==true)return true;status=false;$.ajax({async:false,url:this.build_endpoint_url(name,
id),data:Flakey.util.querystring.build({id:id,versions:JSON.stringify(versions)}),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(){Flakey.status.server_online=true;return status=true},type:"POST"});return status};ServerBackend.prototype.save_to_cache=function(name,id,versions){this.server_cache[name]=this.server_cache[name]||{};return this.server_cache[name][id]=$.extend(true,{},{id:id,versions:versions})};ServerBackend.prototype["delete"]=function(name,
id){var status;status=false;$.ajax({async:false,url:this.build_endpoint_url(name,id),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(){Flakey.status.server_online=true;return status=true},type:"DELETE"});return status};ServerBackend.prototype._query=function(name,query){throw new TypeError("_query not supported on server backend");};ServerBackend.prototype._query_by_id=function(name,id){throw new TypeError("_query_by_id not supported on server backend");
};ServerBackend.prototype._read=function(name){return this.all(name)};ServerBackend.prototype._write=function(name,store){var item,status,_i,_len;status=true;for(_i=0,_len=store.length;_i<_len;_i++){item=store[_i];if(!this.save(name,item.id,item.versions))status=false}return status};return ServerBackend}(Backend);SocketIOBackend=function(_super){__extends(SocketIOBackend,_super);function SocketIOBackend(){var _this=this;this.status=true;this.socket=window.socket=io.connect(Flakey.settings.socketio_server);
this.server_cache={};this.socket.on("sync",function(set){var name,names,obj,_i,_j,_len,_len2,_ref,_results;names=[];for(_i=0,_len=set.length;_i<_len;_i++){obj=set[_i];if(_ref=obj.name,__indexOf.call(names,_ref)<0)names.push(obj.name);_this.save_to_cache(obj.name,obj.id,obj.versions)}_results=[];for(_j=0,_len2=names.length;_j<_len2;_j++){name=names[_j];_results.push(Flakey.models.backend_controller.sync(name))}return _results});this.socket.emit("fetch_all")}SocketIOBackend.prototype.all=function(name){var id,
store,_i,_len,_ref;store=[];this.server_cache[name]=this.server_cache[name]||{};_ref=Object.keys(this.server_cache[name]);for(_i=0,_len=_ref.length;_i<_len;_i++){id=_ref[_i];store.push(this.get_from_cache(name,id))}return store};SocketIOBackend.prototype.get=function(name,id){return this.get_from_cache(name,id)};SocketIOBackend.prototype.get_from_cache=function(name,id){if(this.server_cache[name])if(this.server_cache[name][id])return this.server_cache[name][id]};SocketIOBackend.prototype.save=function(name,
id,versions,force_write){var cached_obj,proposed_obj,_this=this;proposed_obj={id:id,versions:versions};cached_obj=this.get_from_cache(name,id);if(Flakey.util.deep_compare(proposed_obj,cached_obj)&&force_write!==true)return true;this.socket.emit("write",{name:name,id:id,versions:versions},function(){return _this.save_to_cache(name,id,versions)});return this.status};SocketIOBackend.prototype.save_to_cache=function(name,id,versions){this.server_cache[name]=this.server_cache[name]||{};return this.server_cache[name][id]=
...
function(name,store){var item,status,_i,_len;status=true;for(_i=0,_len=store.length;_i<_len;_i++){item=store[_i];if(!this.save(name,item.id,item.versions))status=false}return status};return SocketIOBackend}(Backend);Flakey.models={Model:Model,BackendController:BackendController,backend_controller:null};Controller=function(){function Controller(config){var name,_i,_len,_ref;if(config==null)config={};this.active=this.active||false;this.actions=this.actions||{};this.id=this.id||"";this.class_name=this.class_name||
...
Flakey.controllers={Stack:Stack,Controller:Controller};Template=function(){function Template(eco,name){this.eco=eco;this.name=name}Template.prototype.render=function(context){if(context==null)context={};return this.eco(context)};return Template}();get_template=function(name,tobj){var template;template=tobj.ecoTemplates[name];return new Template(template,name)};Flakey.templates={get_template:get_template,Template:Template};if(typeof module!=="undefined"&&module!==null)module.exports=Flakey;if(window)window.Flakey=
Flakey}).call(this)});
require.define("/controllers/annotare.js",function(require,module,exports,__dirname,__filename){(function(){var $,Flakey,Main,Setting,default_settings,__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};Flakey=require("flakey");$=Flakey.$;Setting=require("../models/Setting");
default_settings=require("../settings");default_settings=default_settings["default_settings"];Main=function(_super){__extends(Main,_super);function Main(config){var Detail,Edit,History,List,NewDocument,Settings,theme;NewDocument=require("./new_document");List=require("./list");Detail=require("./detail");Edit=require("./edit");History=require("./history");Settings=require("./settings");this.id="main-stack";this.class_name="stack";this.controllers={new_document:NewDocument,list:List,detail:Detail,edit:Edit,
history:History,settings:Settings};this.routes={"^/new$":"new_document","^/list$":"list","^/detail$":"detail","^/edit$":"edit","^/history$":"history","^/settings$":"settings"};this["default"]="/list";Main.__super__.constructor.call(this,config);theme=Setting.find({slug:"theme"});theme=theme[0];if(theme!=null&&theme.value!=null)document.body.className=theme.value;else document.body.className=default_settings["Theme"]}return Main}(Flakey.controllers.Stack);module.exports=Main}).call(this)});
require.define("/models/Setting.js",function(require,module,exports,__dirname,__filename){(function(){var Flakey,Setting,__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};Flakey=require("flakey");Setting=function(_super){__extends(Setting,_super);function Setting(){Setting.__super__.constructor.apply(this,
arguments)}Setting.model_name="Setting";Setting.fields=["id","slug","value"];return Setting}(Flakey.models.Model);module.exports=Setting}).call(this)});require.define("/settings.js",function(require,module,exports,__dirname,__filename){(function(){module.exports={growl_hide_after:5E3,growl_effect:"slide",default_settings:{"Theme":"minimal"},user_settings:{"Appearance Settings":{"Theme":["select",["book","minimal"]]}}}}).call(this)});
...
return child};Flakey=require("flakey");$=Flakey.$;ui=require("../lib/uikit");settings=require("../settings");Document=require("../models/Document");Annotation=require("../models/Annotation");Detail=function(_super){__extends(Detail,_super);function Detail(config){this.annotate=__bind(this.annotate,this);this.highlight=__bind(this.highlight,this);this["delete"]=__bind(this["delete"],this);this.edit_note=__bind(this.edit_note,this);this.edit=__bind(this.edit,this);this.render=__bind(this.render,this);
this.id="detail-view";this.class_name="detail view";this.actions={"click .edit":"edit","click .highlighter":"highlight","click .annotate":"annotate","click .delete":"delete","blur .note-detail":"edit_note"};Detail.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("detail",require("../views/detail"))}Detail.prototype.render=function(){var context;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));
this.unbind_actions();return this.bind_actions()};Detail.prototype.edit=function(event){return window.location.hash="#/edit?"+Flakey.util.querystring.build(this.query_params)};Detail.prototype.edit_note=function(event){var content,id,note,target;target=$(event.target);id=target.attr("id").replace("note-detail-","");content=target.html().replace(/\<br\/\>/,"\n\n").replace(/\<([\w\d\/]*)\>/g," ");note=Annotation.get(id);note.attachment=content;note.type="note";return note.save()};Detail.prototype["delete"]=
function(event){var _this=this;return ui.confirm("Be careful!","Are you sure you want to delete this document?").show(function(ok){if(ok){_this.doc["delete"]();return window.location.hash="#/list"}})};Detail.prototype.highlight=function(event){var html,selection;selection=void 0;if(window.getSelection)selection=window.getSelection();else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return ui.error("Well, you were right about this being a bad idea.",
"Please select some text first.").hide(settings.growl_hide_after).effect(settings.growl_effect);else{html=$("#detail-"+this.doc.slug).html();html=this.doc.annotate(selection,html);return $("#detail-"+this.doc.slug).html(html)}};Detail.prototype.annotate=function(event){var html,options,selection;selection=void 0;if(window.getSelection)selection=window.getSelection();else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return ui.error("Well, you were right about this being a bad idea.",
"Please select some text first.").hide(settings.growl_hide_after).effect(settings.growl_effect);else{html=$("#detail-"+this.doc.slug).html();options={input:true,animate:true};html=this.doc.annotate(selection,html,"Click here to edit this note.");return $("#detail-"+this.doc.slug).html(html)}};return Detail}(Flakey.controllers.Controller);module.exports=Detail}).call(this)});
...
this.actions={"click .save":"save","click .discard":"discard","click .delete":"delete_note","keyup #editor":"autosave"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.autosave=function(event){event.preventDefault();return localStorage[this.autosave_key()]=$("#editor").val()};Edit.prototype.autosave_key=function(){return"autosave-draft-"+this.id};Edit.prototype.render=function(){var context,_this=this;if(!this.query_params.id)return;
this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){if(ok)return $("#editor").val(localStorage[_this.autosave_key()])});this.unbind_actions();this.bind_actions();return $("#editor").autoResize({extraSpace:100,maxHeight:9E3}).blur()};Edit.prototype.save=
function(event){event.preventDefault();this.doc.base_text=$("#editor").val();this.doc.save();ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(settings.growl_hide_after).effect(settings.growl_effect);return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok){delete localStorage[_this.autosave_key()];
return window.location.hash="#/list"}})};Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};return Edit}(Flakey.controllers.Controller);module.exports=Edit}).call(this)});
...
__out.push(__sanitize(this.doc.slug));__out.push('">\n      <h4>Version from <em id="history-time">');__out.push(__sanitize(this.time.toLocaleString()));__out.push('</em></h4>\n      <h1 class="name">');__out.push(__sanitize(this.doc.name));__out.push("</h1>\n      ");html=this.html!=null?this.html:this.doc.render();__out.push('\n      <section id="history-content">\n         ');__out.push(this.doc.draw_annotations(html));__out.push("\n      </section>\n    </article>\n    \n    <aside>\n      &nbsp;\n    </aside>\n  </section>\n</div>")}).call(this)}).call(__obj);
__obj.safe=__objSafe,__obj.escape=__escape;return __out.join("")}}).call(this)});
require.define("/controllers/settings.js",function(require,module,exports,__dirname,__filename){(function(){var $,Flakey,Setting,Settings,setting_types,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;
return child};Flakey=require("flakey");$=Flakey.$;setting_types=require("../settings");Setting=require("../models/Setting");Settings=function(_super){__extends(Settings,_super);function Settings(config){this.save=__bind(this.save,this);this.id="settings-view";this.class_name="settings view";this.actions={"change .user-setting":"save"};Settings.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("settings",require("../views/settings"))}Settings.prototype.render=function(){var context;
context={types:setting_types,Setting:Setting};this.html(this.tmpl.render(context));this.unbind_actions();return this.bind_actions()};Settings.prototype.save=function(event){event.preventDefault();return $(".user-setting").each(function(){var setting,settings;settings=Setting.find({slug:$(this).attr("data-slug")});if(settings.length===0)setting=new Setting({slug:$(this).attr("data-slug")});else setting=settings[0];setting.value=$(this).val();return setting.save()})};return Settings}(Flakey.controllers.Controller);
module.exports=Settings}).call(this)});
require.define("/views/settings.js",function(require,module,exports,__dirname,__filename){(function(){this.ecoTemplates||(this.ecoTemplates={});this.ecoTemplates["settings"]=function(__obj){if(!__obj)__obj={};var __out=[],__capture=function(callback){var out=__out,result;__out=[];callback.call(this);result=__out.join("");__out=out;return __safe(result)},__sanitize=function(value){if(value&&value.ecoSafe)return value;else if(typeof value!=="undefined"&&value!=null)return __escape(value);else return""},
__safe,__objSafe=__obj.safe,__escape=__obj.escape;__safe=__obj.safe=function(value){if(value&&value.ecoSafe)return value;else{if(!(typeof value!=="undefined"&&value!=null))value="";var result=new String(value);result.ecoSafe=true;return result}};if(!__escape)__escape=__obj.escape=function(value){return(""+value).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")};(function(){(function(){var category,current,name,option,setting,settings,slug,_i,_len,_ref,_ref2,__hasProp=
Object.prototype.hasOwnProperty;__out.push('<div class="tool-bar-wrap">\n  <nav id="tool-bar">\n    <a href="/" class="apply">Apply</a>\n  </nav>\n</div>\n\n<div class="wrap">\n  <section class="one-column">\n    <h1>Settings</h1>\n    <form action="#" method="POST">\n      ');_ref=this.types.user_settings;for(category in _ref){if(!__hasProp.call(_ref,category))continue;settings=_ref[category];__out.push("\n        <h3>");__out.push(__sanitize(category));__out.push("</h3>\n        ");for(name in settings){if(!__hasProp.call(settings,
name))continue;setting=settings[name];__out.push('\n          <div class="setting">\n            <label for="setting-');__out.push(__sanitize(name.replace(" ","")));__out.push('">');__out.push(__sanitize(name));__out.push("</label>\n            \n            ");slug=name.replace(" ","-").toLowerCase();__out.push("\n            ");current=this.Setting.find({slug:slug});__out.push("\n            ");current=current[0];__out.push("\n            ");current=current!=null&&current.value!=null?current.value:
this.types.default_settings[name];__out.push("\n            ");current=current!=null?current:"";__out.push("\n            \n            ");if(setting[0]==="select"){__out.push('\n            \n              <select id="setting-');__out.push(__sanitize(name.replace(" ","")));__out.push('" name="setting-');__out.push(__sanitize(name.replace(" ","")));__out.push('" data-slug="');__out.push(__sanitize(slug));__out.push('" class="user-setting">\n                ');_ref2=setting[1];for(_i=0,_len=_ref2.length;_i<
_len;_i++){option=_ref2[_i];__out.push("\n                  ");if(current===option){__out.push('\n                    <option value="');__out.push(__sanitize(option));__out.push('" selected="selected">');__out.push(__sanitize(option));__out.push("</option>\n                  ")}else{__out.push('\n                    <option value="');__out.push(__sanitize(option));__out.push('">');__out.push(__sanitize(option));__out.push("</option>\n                  ")}__out.push("\n                ")}__out.push("\n              </select>\n              \n            ")}else{__out.push('\n            \n              <input type="');
__out.push(__sanitize(setting[0]));__out.push('" id="setting-');__out.push(__sanitize(name.replace(" ","")));__out.push('" name="setting-');__out.push(__sanitize(name.replace(" ","")));__out.push('" data-slug="');__out.push(__sanitize(slug));__out.push('" class="user-setting" value="');__out.push(__sanitize(current));__out.push('" />\n              \n            ')}__out.push("\n          </div>\n        ")}__out.push("\n      ")}__out.push("\n    </form>\n  </section>\n</div>")}).call(this)}).call(__obj);
__obj.safe=__objSafe,__obj.escape=__escape;return __out.join("")}}).call(this)});
...
window.Annotare=new Annotare;return annotare.make_active()});module.exports=App}).call(this)});require("/index.js");
