...
return child};Flakey=require("flakey");Showdown=require("../lib/showdown");console.log(Showdown);Classify=require("../lib/classify");console.log(Classify);Annotation=require("./Annotation");Document=function(_super){__extends(Document,_super);function Document(){this.render=__bind(this.render,this);this.get_notes=__bind(this.get_notes,this);this.generate_slug=__bind(this.generate_slug,this);this.draw_annotations=__bind(this.draw_annotations,this);this.annotate=__bind(this.annotate,this);Document.__super__.constructor.apply(this,
arguments)}Document.model_name="Document";Document.fields=["id","name","slug","base_text","annotations"];Document.prototype.annotate=function(selection,html,attachment){var note,type;if(attachment===void 0)type="highlight";else type="note";note=new Annotation({text:selection,type:type,attachment:attachment,document:this.id});note.save();if(!this.annotations||this.annotations.constructor!==Array)this.annotations=[];this.annotations.push(note.id);this.save();return note.apply(html)};Document.prototype["delete"]=
...
this.annotations.constructor!==Array)this.annotations=[];notes=[];_ref=this.annotations;for(_i=0,_len=_ref.length;_i<_len;_i++){id=_ref[_i];note=Annotation.get(id);if(note!=null)notes.push(note)}return notes};Document.prototype.render=function(){var classy_converter,converter,html,testclassify;converter=new Showdown.converter;console.log("Showdown Object");console.log(converter);classy_converter=new Classify;testclassify=classy_converter.converter("Testing");console.log("Classy Object");console.log(classy_converter);
console.log(testclassify);html=converter.makeHtml(this.base_text);return html};return Document}(Flakey.models.Model);module.exports=Document}).call(this)});
...
require.define("/lib/classify.js",function(require,module,exports,__dirname,__filename){var Classify={};Classify.name=function(){return alert("I'm Classy!")};Classify.converter=function(text){text="Classified! "+text;return text};module.export=Classify});
...
Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");ui=require("../lib/uikit");Document=require("../models/Document");Edit=function(_super){__extends(Edit,_super);function Edit(config){this.loggin=__bind(this.loggin,this);this.delete_note=__bind(this.delete_note,this);this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.render=__bind(this.render,this);this.id="edit-view";this.class_name="edit_document view";this.actions={"click .save":"save","click .discard":"discard",
"click .delete":"delete_note"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.render=function(){var context;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));this.unbind_actions();this.bind_actions();return $("#editor").autoResize({extraSpace:100,maxHeight:9E3})};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=
$("#editor").val();this.doc.save();ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok)return window.location.hash="#/list"})};
Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};Edit.prototype.loggin=function(event){event.preventDefault();return console.log(this.autosave_key())};return Edit}(Flakey.controllers.Controller);module.exports=Edit}).call(this)});
