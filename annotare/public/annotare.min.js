...
"Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok){delete localStorage[_this.autosave_key()];return window.location.hash="#/list"}})};Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};
return Edit}();module.exports=Edit}).call(this)});
