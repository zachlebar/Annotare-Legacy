...
require.define("/controllers/new_document.js",function(require,module,exports,__dirname,__filename){(function(){var $,Document,Flakey,NewDocument,autoresize,ui,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;
return child};Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");ui=require("../lib/uikit");Document=require("../models/Document");NewDocument=function(_super){__extends(NewDocument,_super);function NewDocument(config){this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.id="new-document-view";this.class_name="new_document view";this.actions={"click .save":"save","click .discard":"discard"};NewDocument.__super__.constructor.call(this,config);this.tmpl=
Flakey.templates.get_template("new_document",require("../views/new_document"))}NewDocument.prototype.render=function(){this.html(this.tmpl.render({title:this.query_params.title}));this.unbind_actions();this.bind_actions();$("#new-doc-editor").autoResize({extraSpace:100,maxHeight:2E3});return $("#new-doc-editor").blur()};NewDocument.prototype.save=function(params){var doc,text;text=$("#new-doc-editor").val();if(text.length>0){doc=new Document({base_text:text});doc.save();ui.info("Everything's Shiny Capt'n!",
'"'+doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build({id:doc.id})}};NewDocument.prototype.discard=function(params){return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard this document?").show(function(ok){if(ok)return window.location.hash="#/list"})};return NewDocument}(Flakey.controllers.Controller);module.exports=NewDocument}).call(this)});
...
__obj.safe=__objSafe,__obj.escape=__escape;return __out.join("")}}).call(this)});
require.define("/controllers/edit.js",function(require,module,exports,__dirname,__filename){(function(){var $,Document,Edit,Flakey,autoresize,ui,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};
Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");ui=require("../lib/uikit");Document=require("../models/Document");Edit=function(_super){__extends(Edit,_super);function Edit(config){this.delete_note=__bind(this.delete_note,this);this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.render=__bind(this.render,this);this.autosave=__bind(this.autosave,this);this.id="edit-view";this.class_name="edit_document view";this.actions={"click .save":"save","click .discard":"discard",
"click .delete":"delete_note","keyup #edit-editor":"autosave"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.autosave=function(event){event.preventDefault();return localStorage[this.autosave_key()]=$("#edit-editor").val()};Edit.prototype.autosave_key=function(){return"autosave-draft-"+this.doc.id};Edit.prototype.render=function(){var context,_this=this;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);
context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){console.log("Inside anonymous function...");if(ok){console.log("Restoring ...");return $("#edit-editor").val(localStorage[_this.autosave_key()])}else return delete localStorage[_this.autosave_key()]});this.unbind_actions();this.bind_actions();
return $("#edit-editor").autoResize({extraSpace:100,maxHeight:9E3})};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=$("#edit-editor").val();this.doc.save();delete localStorage[this.autosave_key()];ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();
return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok){delete localStorage[_this.autosave_key()];return window.location.hash="#/list"}})};Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);
return $(event.target).parent().slideUp()}})};return Edit}(Flakey.controllers.Controller);module.exports=Edit}).call(this)});
