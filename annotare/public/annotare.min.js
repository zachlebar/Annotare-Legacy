...
parent.prototype;return child};Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");ui=require("../lib/uikit");Classify=require("../lib/classify");Document=require("../models/Document");NewDocument=function(_super){__extends(NewDocument,_super);function NewDocument(config){this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.autosave=__bind(this.autosave,this);this.id="new-document-view";this.class_name="new_document view";this.actions={"click .save":"save",
"click .discard":"discard","keyup #new-doc-editor":"autosave"};NewDocument.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("new_document",require("../views/new_document"))}NewDocument.prototype.autosave=function(event){event.preventDefault();return localStorage[this.autosave_key()]=$("#new-doc-editor").val()};NewDocument.prototype.autosave_key=function(){return"autosave-draft-new-doc"};NewDocument.prototype.auto_resize=function(){return $("#new-doc-editor").autoResize({extraSpace:100,
maxHeight:9E3})};NewDocument.prototype.render=function(){this.html(this.tmpl.render({title:this.query_params.title}));this.unbind_actions();this.bind_actions();$("#new-doc-editor").autoResize({extraSpace:100,maxHeight:9E3});return $("#new-doc-editor").blur()};NewDocument.prototype.save=function(params){var class_converter,doc,dupes,html,text;text=$("#new-doc-editor").val();if(text.length>0)doc=new Document({base_text:text});html=doc.render();class_converter=new Classify.converter;doc.name=class_converter.extractClass(html,
"title");doc.slug=class_converter.extractClass(html,"slug");if(!doc.slug)doc.generate_slug();dupes=Document.find({slug:doc.slug});if(dupes.length>0){ui.info("A document with that name already exists!").hide(5E3).effect("slide");window.location.hash="#/new";if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0){$("#new-doc-editor").val(localStorage[this.autosave_key()]);return this.auto_resize()}}else{doc.save();if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>
0)delete localStorage[this.autosave_key()];ui.info("Everything's Shiny Capt'n!",'"'+doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build({slug:doc.slug})}};NewDocument.prototype.discard=function(params){ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard this document?").show(ok)(function(){});if(ok)return window.location.hash="#/list"};return NewDocument}(Flakey.controllers.Controller);
...
this.unbind_actions();this.bind_actions();return this.auto_resize()};Edit.prototype.auto_resize=function(){return $("#edit-editor").autoResize({extraSpace:100,maxHeight:9E3})};Edit.prototype.save=function(event){var class_converter,dupe_doc,dupes,tmp_html,_i,_len;event.preventDefault();this.doc.base_text=$("#edit-editor").val();tmp_html=this.doc.render();class_converter=new Classify.converter;this.doc.name=class_converter.extractClass(tmp_html,"title");this.doc.slug=class_converter.extractClass(tmp_html,
"slug");if(!this.doc.slug)this.doc.slug=this.doc.generate_slug();dupes=Document.find({slug:this.doc.slug});if(dupes.length>0)for(_i=0,_len=dupes.length;_i<_len;_i++){dupe_doc=dupes[_i];if(dupe_doc.id!==this.doc.id){ui.info("A document with that name already exists!").hide(5E3).effect("slide");return}}this.doc.save();this.query_params.slug=this.doc.slug;if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)delete localStorage[this.autosave_key()];ui.info("Everything's Shiny Capt'n!",
'"'+this.doc.name+'" was successfully saved.').hide(5E3).effect("slide");return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){});if(ok)return window.location.hash="#/list"};Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();
return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};return Edit}(Flakey.controllers.Controller);module.exports=Edit}).call(this)});
