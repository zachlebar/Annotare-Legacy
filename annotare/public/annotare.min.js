...
Flakey.util={async:function(fn){return setTimeout(fn,0)},deep_compare:function(a,b){var compare_objects;if(typeof a!==typeof b)return false;compare_objects=function(a,b){var key,value;for(key in a){value=a[key];if(!(b[key]!=null))return false}for(key in b){value=b[key];if(!(a[key]!=null))return false}for(key in a){value=a[key];if(value)switch(typeof value){case "object":if(!compare_objects(value,b[key]))return false;break;default:if(value!==b[key])return false}else if(b[key])return false}return true};
...
Model.get=function(id){var m,obj;obj=Flakey.models.backend_controller.get(this.model_name,id);if(!obj)return;m=new this;m["import"](obj);return m};Model.search=function(re,fields){var ar,item,key,m,query,set,_i,_j,_len,_len2;if(fields==null)fields=this.fields;query={};for(_i=0,_len=fields.length;_i<_len;_i++){key=fields[_i];query[key]=re}ar=Flakey.models.backend_controller.search(this.model_name,query);if(!ar.length)return[];set=[];for(_j=0,_len2=ar.length;_j<_len2;_j++){item=ar[_j];m=new this;m["import"](item);
set.push(m)}return set};Model.prototype["delete"]=function(){var event_key;Flakey.models.backend_controller["delete"](this.constructor.model_name,this.id);event_key="model_"+this.constructor.model_name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)};Model.prototype.rollback=function(version_id){var exists,i,latest,version,_i,_j,_len,_len2,_ref,_ref2;if(parseInt(version_id)<this.versions.length){_ref=Array(parseInt(version_id));for(_i=0,_len=_ref.length;_i<_len;_i++){i=_ref[_i];
this.pop_version()}}else{exists=false;_ref2=this.versions;for(_j=0,_len2=_ref2.length;_j<_len2;_j++){version=_ref2[_j];if(version.version_id===version_id)exists=true}if(!exists)throw new ReferenceError("Version "+version_id+" does not exist.");latest=this.versions[this.versions.length-1];while(latest.version_id!==version_id&&this.versions.length>1){this.pop_version();latest=this.versions[this.versions.length-1]}}this["import"]({id:this.id,versions:this.versions});return this.write()};Model.prototype.save=
function(callback){var diff,new_obj,old_obj;new_obj=this["export"]();old_obj=this.evolve();diff=this.diff(new_obj,old_obj);if(Object.keys(diff).length>0){this.push_version(diff);this.write(callback)}else if(callback!=null)callback();return true};Model.prototype.diff=function(new_obj,old_obj){var key,patches,save,_i,_len,_ref;save={};_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){key=_ref[_i];if(!Flakey.util.deep_compare(new_obj[key],old_obj[key]))switch(new_obj[key].constructor){case Object:save[key]=
$.extend(true,{},new_obj[key]);break;case Array:save[key]=$.extend(true,[],new_obj[key]);break;case String:old_obj[key]=old_obj[key]!=null?old_obj[key].toString():"";if(Flakey.settings.diff_text){patches=Flakey.diff_patch.patch_make(old_obj[key],new_obj[key]);save[key]={constructor:"Patch",patch_text:Flakey.diff_patch.patch_toText(patches)}}else save[key]=new_obj[key];break;default:save[key]=new_obj[key]}}return save};Model.prototype["export"]=function(){var field,obj,_i,_len,_ref;obj={};_ref=this.constructor.fields;
...
Backend.prototype.find=function(name,query,full_text){if(full_text==null)full_text=false;if(full_text)return this._search(name,query);else return this._query(name,query)};Backend.prototype.get=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,id);if(index===-1)return;return store[index]};Backend.prototype.save=function(name,id,versions){var index,obj,store;store=this._read(name);if(!store)store=[];index=this._query_by_id(name,id);obj={id:id,versions:versions};if(index===
-1)store.push(obj);else store[index]=obj;return this._write(name,store)};Backend.prototype._query=function(name,query){var key,match,obj,rendered,set,store,value,_i,_len;store=this._read(name);if(!store)return[];set=[];for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];rendered=this._render_obj(obj);match=true;for(key in query){if(!__hasProp.call(query,key))continue;value=query[key];if(rendered[key]!==value)match=false}if(match)set.push(obj)}return set};Backend.prototype._query_by_id=function(name,
id){var i,obj,store,_i,_len;store=this._read(name);if(!store)return-1;i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];if(obj.id===id)return i;i++}return-1};Backend.prototype._render_obj=function(obj){var key,output,patches,rev,value,_i,_len,_ref,_ref2;output={};_ref=obj.versions;for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];_ref2=rev.fields;for(key in _ref2){if(!__hasProp.call(_ref2,key))continue;value=_ref2[key];switch(value.constructor){case "Patch":patches=Flakey.diff_patch.patch_fromText(value.patch_text);
obj[key]=Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0];break;case Object:obj[key]=$.extend(true,{},value);break;case Array:obj[key]=$.extend(true,[],value);break;default:obj[key]=value}}}return obj};Backend.prototype._search=function(name,query){var key,match,obj,rendered,set,store,value,_i,_len;store=this._read(name);if(!store)return[];for(key in query){if(!__hasProp.call(query,key))continue;value=query[key];query[key]=new RegExp(value,"g")}set=[];for(_i=0,_len=store.length;_i<_len;_i++){obj=
store[_i];rendered=this._render_obj(obj);match=false;for(key in query){if(!__hasProp.call(query,key))continue;value=query[key];if(value.exec(rendered[key]))match=true}if(match)set.push(obj)}return set};return Backend}();MemoryBackend=function(){__extends(MemoryBackend,Backend);function MemoryBackend(){if(!window.memcache)window.memcache={}}MemoryBackend.prototype._read=function(name){return window.memcache[name]};MemoryBackend.prototype._write=function(name,store){window.memcache[name]=store;return true};
return MemoryBackend}();LocalBackend=function(){__extends(LocalBackend,Backend);function LocalBackend(){this.prefix="flakey-"}LocalBackend.prototype._read=function(name){var store;if(!localStorage[this.prefix+name])localStorage[this.prefix+name]=JSON.stringify([]);store=JSON.parse(localStorage[this.prefix+name]);return store};LocalBackend.prototype._write=function(name,store){localStorage[this.prefix+name]=JSON.stringify(store);return true};return LocalBackend}();ServerBackend=function(){__extends(ServerBackend,
Backend);function ServerBackend(){this.server_cache={}}ServerBackend.prototype.build_endpoint_url=function(name,id,params){var querystring,url;url=""+Flakey.settings.base_model_endpoint+"/"+name;if(id!=null)url+="/"+id;if(params!=null&&params.constructor===Object){querystring=Flakey.util.querystring.build(params);url+="?"+querystring}return url};ServerBackend.prototype.all=function(name){var obj,store,_i,_len;store=false;$.ajax({async:false,url:this.build_endpoint_url(name),dataType:"json",error:function(){return Flakey.status.server_online=
false},success:function(data){Flakey.status.server_online=true;return store=data},type:"GET"});for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];this.save_to_cache(name,obj.id,obj.versions)}return store};ServerBackend.prototype.get=function(name,id){var obj;obj=false;$.ajax({async:false,url:this.build_endpoint_url(name,id),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return obj=data},type:"GET"});this.save_to_cache(name,
...
Flakey}).call(this)});
require.define("/controllers/annotare.js",function(require,module,exports,__dirname,__filename){(function(){var $,Flakey,Main,Setting,default_settings;var __hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};Flakey=require("flakey");$=Flakey.$;Setting=require("../models/Setting");
default_settings=require("../settings");default_settings=default_settings["default_settings"];Main=function(){__extends(Main,Flakey.controllers.Stack);function Main(config){var Detail,Edit,History,List,NewDocument,Settings,theme;NewDocument=require("./new_document");List=require("./list");Detail=require("./detail");Edit=require("./edit");History=require("./history");Settings=require("./settings");this.id="main-stack";this.class_name="stack";this.controllers={new_document:NewDocument,list:List,detail:Detail,
edit:Edit,history:History,settings:Settings};this.routes={"^/new$":"new_document","^/list$":"list","^/detail$":"detail","^/edit$":"edit","^/history$":"history","^/settings$":"settings"};this["default"]="/list";Main.__super__.constructor.call(this,config);theme=Setting.find({slug:"theme"});theme=theme[0];if(theme!=null&&theme.value!=null)document.body.className=theme.value;else document.body.className=default_settings["Theme"]}return Main}();module.exports=Main}).call(this)});
require.define("/models/Setting.js",function(require,module,exports,__dirname,__filename){(function(){var Flakey,Setting;var __hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};Flakey=require("flakey");Setting=function(){__extends(Setting,Flakey.models.Model);
function Setting(){Setting.__super__.constructor.apply(this,arguments)}Setting.model_name="Setting";Setting.fields=["id","slug","value"];return Setting}();module.exports=Setting}).call(this)});require.define("/settings.js",function(require,module,exports,__dirname,__filename){module.exports={growl_hide_after:5E3,growl_effect:"slide",default_settings:{"Theme":"minimal"},user_settings:{"Appearance Settings":{"Theme":["select",["book","minimal"]]}}}});
...
Flakey=require("flakey");$=Flakey.$;Document=require("../models/Document");List=function(){__extends(List,Flakey.controllers.Controller);function List(config){this.search=__bind(this.search,this);this.render=__bind(this.render,this);this.id="list-view";this.class_name="list view";this.actions={"click .document":"select_doc","keyup #search-box":"search"};List.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("list",require("../views/list"))}List.prototype.render=function(){var context,
documents;if(this.query_params.q!=null&&this.query_params.q.length>0)documents=Document.search(this.query_params.q);else documents=Document.all();context={list:documents};this.html(this.tmpl.render(context));this.unbind_actions();this.bind_actions();return $("#search-box").val(this.query_params.q).focus()};List.prototype.select_doc=function(event){var id;id=$(event.currentTarget).attr("id").replace("document-","");return window.location.hash="#/detail?"+Flakey.util.querystring.build({id:id})};List.prototype.search=
function(event){event.preventDefault();return Flakey.util.querystring.update({q:$("#search-box").val()},true)};return List}();module.exports=List}).call(this)});
...
__obj.safe=__objSafe,__obj.escape=__escape;return __out.join("")}}).call(this)});
require.define("/controllers/settings.js",function(require,module,exports,__dirname,__filename){(function(){var $,Flakey,Setting,Settings,setting_types;var __bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;
return child};Flakey=require("flakey");$=Flakey.$;setting_types=require("../settings");Setting=require("../models/Setting");Settings=function(){__extends(Settings,Flakey.controllers.Controller);function Settings(config){this.save=__bind(this.save,this);this.id="settings-view";this.class_name="settings view";this.actions={"change .user-setting":"save"};Settings.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("settings",require("../views/settings"))}Settings.prototype.render=
function(){var context;context={types:setting_types,Setting:Setting};this.html(this.tmpl.render(context));this.unbind_actions();return this.bind_actions()};Settings.prototype.save=function(event){event.preventDefault();return $(".user-setting").each(function(){var setting,settings;settings=Setting.find({slug:$(this).attr("data-slug")});if(settings.length===0)setting=new Setting({slug:$(this).attr("data-slug")});else setting=settings[0];setting.value=$(this).val();return setting.save()})};return Settings}();
