...
Model.get=function(id){var m,obj;obj=Flakey.models.backend_controller.get(this.model_name,id);if(!obj)return;m=new this;m["import"](obj);return m};Model.search=function(re,fields){var ar,item,key,m,query,set,_i,_j,_len,_len2;if(fields==null)fields=this.fields;query={};for(_i=0,_len=fields.length;_i<_len;_i++){key=fields[_i];query[key]=re}console.log(query);ar=Flakey.models.backend_controller.search(this.model_name,query);if(!ar.length)return[];set=[];for(_j=0,_len2=ar.length;_j<_len2;_j++){item=ar[_j];
m=new this;m["import"](item);set.push(m)}return set};Model.prototype["delete"]=function(){var event_key;Flakey.models.backend_controller["delete"](this.constructor.model_name,this.id);event_key="model_"+this.constructor.model_name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)};Model.prototype.rollback=function(version_id){var exists,i,latest,version,_i,_j,_len,_len2,_ref,_ref2;if(parseInt(version_id)<this.versions.length){_ref=Array(parseInt(version_id));for(_i=0,_len=_ref.length;_i<
_len;_i++){i=_ref[_i];this.pop_version()}}else{exists=false;_ref2=this.versions;for(_j=0,_len2=_ref2.length;_j<_len2;_j++){version=_ref2[_j];if(version.version_id===version_id)exists=true}if(!exists)throw new ReferenceError("Version "+version_id+" does not exist.");latest=this.versions[this.versions.length-1];while(latest.version_id!==version_id&&this.versions.length>1){this.pop_version();latest=this.versions[this.versions.length-1]}}this["import"]({id:this.id,versions:this.versions});return this.write()};
Model.prototype.save=function(callback){var diff,new_obj,old_obj;new_obj=this["export"]();old_obj=this.evolve();diff=this.diff(new_obj,old_obj);if(Object.keys(diff).length>0){this.push_version(diff);this.write(callback)}else if(callback!=null)callback();return true};Model.prototype.diff=function(new_obj,old_obj){var key,patches,save,_i,_len,_ref;save={};_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){key=_ref[_i];if(!Flakey.util.deep_compare(new_obj[key],old_obj[key]))switch(new_obj[key].constructor){case Object:save[key]=
...
output[key]=Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0]}else output[key]=value}}return output};Backend.prototype._search=function(name,query){var key,match,obj,rendered,set,store,value,_i,_len;store=this._read(name);if(!store)return[];for(key in query){if(!__hasProp.call(query,key))continue;value=query[key];query[key]=new RegExp(value,"g")}set=[];for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];rendered=this._render_obj(obj);console.log(rendered);match=false;for(key in query){if(!__hasProp.call(query,
key))continue;value=query[key];match=true}if(match)set.push(obj)}return set};return Backend}();MemoryBackend=function(){__extends(MemoryBackend,Backend);function MemoryBackend(){if(!window.memcache)window.memcache={}}MemoryBackend.prototype._read=function(name){return window.memcache[name]};MemoryBackend.prototype._write=function(name,store){window.memcache[name]=store;return true};return MemoryBackend}();LocalBackend=function(){__extends(LocalBackend,Backend);function LocalBackend(){this.prefix=
...
id,params){var querystring,url;url=""+Flakey.settings.base_model_endpoint+"/"+name;if(id!=null)url+="/"+id;if(params!=null&&params.constructor===Object){querystring=Flakey.util.querystring.build(params);url+="?"+querystring}return url};ServerBackend.prototype.all=function(name){var obj,store,_i,_len;store=false;$.ajax({async:false,url:this.build_endpoint_url(name),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return store=
data},type:"GET"});for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];this.save_to_cache(name,obj.id,obj.versions)}return store};ServerBackend.prototype.get=function(name,id){var obj;obj=false;$.ajax({async:false,url:this.build_endpoint_url(name,id),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return obj=data},type:"GET"});this.save_to_cache(name,obj.id,obj.versions);return obj};ServerBackend.prototype.get_from_cache=
...
this.get_from_cache(name,id);if(Flakey.util.deep_compare(proposed_obj,cached_obj)&&force_write!==true)return true;this.socket.emit("write",{name:name,id:id,versions:versions},function(){return _this.save_to_cache(name,id,versions)});return this.status};SocketIOBackend.prototype.save_to_cache=function(name,id,versions){this.server_cache[name]=this.server_cache[name]||{};return this.server_cache[name][id]=$.extend(true,{},{id:id,versions:versions})};SocketIOBackend.prototype["delete"]=function(name,
id){this.socket.emit("delete",{name:name,id:id});return this.status};SocketIOBackend.prototype._query=function(name,query){throw new TypeError("_query not supported on socketio backend");};SocketIOBackend.prototype._query_by_id=function(name,id){throw new TypeError("_query_by_id not supported on socketio backend");};SocketIOBackend.prototype._read=function(name){return this.all(name)};SocketIOBackend.prototype._write=function(name,store){var item,status,_i,_len;status=true;for(_i=0,_len=store.length;_i<
_len;_i++){item=store[_i];if(!this.save(name,item.id,item.versions))status=false}return status};return SocketIOBackend}();Flakey.models={Model:Model,BackendController:BackendController,backend_controller:null};Controller=function(){function Controller(config){var name,_i,_len,_ref;if(config==null)config={};this.active=this.active||false;this.actions=this.actions||{};this.id=this.id||"";this.class_name=this.class_name||"";this.parent=this.parent||null;this.container=this.container||null;this.container_html=
this.container_html||"";this.subcontrollers=this.subcontrollers||[];this.query_params=this.query_params||{};this.container=$(document.createElement("div"));this.container.html(this.container_html);this.parent=config.parent||Flakey.settings.container;this.parent.append(this.container);this.container.attr("id",this.id);_ref=this.class_name.split(" ");for(_i=0,_len=_ref.length;_i<_len;_i++){name=_ref[_i];this.container.addClass(name)}}Controller.prototype.append=function(){var Contr,contr,_i,_len,_results;
_results=[];for(_i=0,_len=arguments.length;_i<_len;_i++){Contr=arguments[_i];contr=new Contr({parent:this.parent});_results.push(this.subcontrollers.push(contr))}return _results};Controller.prototype.render=function(){return this.html("")};Controller.prototype.bind_actions=function(){var action,fn,key,key_parts,selector,_ref,_results;_ref=this.actions;_results=[];for(key in _ref){if(!__hasProp.call(_ref,key))continue;fn=_ref[key];key_parts=key.split(" ");action=key_parts.shift();selector=key_parts.join(" ");
_results.push($(selector).bind(action,this[fn]))}return _results};Controller.prototype.unbind_actions=function(){var action,fn,key,key_parts,selector,_ref,_results;_ref=this.actions;_results=[];for(key in _ref){if(!__hasProp.call(_ref,key))continue;fn=_ref[key];key_parts=key.split(" ");action=key_parts.shift();selector=key_parts.join(" ");_results.push($(selector).unbind(action))}return _results};Controller.prototype.html=function(htm){this.container_html=htm;this.container.html(this.container_html);
...
Flakey=require("flakey");$=Flakey.$;Document=require("../models/Document");List=function(){__extends(List,Flakey.controllers.Controller);function List(config){this.search=__bind(this.search,this);this.render=__bind(this.render,this);this.id="list-view";this.class_name="list view";this.actions={"click .document":"select_doc","keyup #search-box":"search"};List.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("list",require("../views/list"))}List.prototype.render=function(documents){var context;
if(!(documents!=null)||documents.constructor!==Array)documents=Document.all();context={list:documents};this.html(this.tmpl.render(context));this.unbind_actions();return this.bind_actions()};List.prototype.select_doc=function(event){var id;id=$(event.currentTarget).attr("id").replace("document-","");return window.location.hash="#/detail?"+Flakey.util.querystring.build({id:id})};List.prototype.search=function(event){var docs,query;event.preventDefault();query=$("#search-box").val();docs=Document.search(query);
this.render(docs);return $("#search-box").val(query).focus()};return List}();module.exports=List}).call(this)});
...
return Detail}();module.exports=Detail}).call(this)});
